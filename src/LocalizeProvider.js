...
export class LocalizeProvider extends Component {
  ...
  constructor(props) {
    ...
    const dispatch = this.props.store ? this.props.store.dispatch : this.dispatch.bind(this)
    this.getContextPropsSelector = getContextPropsFromState(dispatch)
    const initialState =
      this.props.initialize !== undefined
        ? localizeReducer(undefined, { type: INITIALIZE, payload: this.props.initialize })
  ...
  dispatch(action) {
    this.setState(prevState => {
      return { localize: localizeReducer(prevState.localize, action) }
    })
  }

  render() {
    this.contextProps = this.getContextPropsSelector(this.state.localize)
    return <LocalizeContext.Provider value={this.contextProps}>{this.props.children}</LocalizeContext.Provider>
  }
}
